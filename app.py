...
def home():
    if request.method == 'POST':
        ...
        if not symptoms:
            message = "Please select at least one symptom from Disease Category's checkboxes"
            return render_template('index.html', message=message)
        else:
            ...
            return render_template('index.html', predicted_disease=predicted_disease, dis_des=dis_des,
                                   my_precautions=my_precautions, medications=medications, my_diet=rec_diet,
                                   workout=workout, selected_symptoms=symptoms)
    ...
    if request.method == 'POST':
        ...
        for symptom in symptoms1:
            try:
                ...
                print(f"Error: '{symptom}' is not a valid symptom.")
                message = "Either spelling of one or more symptom didn't matched in our symptoms DB, or its empty. "
                return render_template('index.html', message=message)

        #return symptom_indices
        ...
        if symptoms =="Symptoms":
            ...
            return render_template('index.html', predicted_disease=predicted_disease, dis_des=dis_des,
                                   my_precautions=my_precautions, medications=medications, my_diet=rec_diet,
                                   workout=workout, searched_symptoms=symptoms)
    return render_template('index.html')
